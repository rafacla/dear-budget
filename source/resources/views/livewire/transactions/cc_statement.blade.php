<div class="fixed z-10 inset-0 overflow-y-auto ease-out duration-400" x-cloak>
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all
            ...
            @endif
            @if ($pickBudgetTransactionId != '' && $pickBudgetTransactionId != null)    
            <div class="rounded bg-white shadow p-8 absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
                <input type="text" hidden wire:model="pickBudgetTransactionId">
            ...
            <p class="font-bold px-4 py-4">{{ __('Credit Card Statement of ')}}
                <input class="font-bold border-0 px-0 w-auto" type="month" wire:model="statementBudgetDate" wire:change="selectStatementBudgetDate()" id="currentDate">
            </p>
            <div class="flex">
                @php
                    $payments = 0;
                ...
                <div class="w-full grid grid-cols-4 px-4">
                    @foreach ($statementTransactions as $item)
                        ...
                        <div class="group text-sm px-2">
                            ...
                            <span title="{{$item['transactions_journal']['description']}}">{{ mb_strimwidth($item['transactions_journal']['description'], 0, 25, '...') }}</span>
